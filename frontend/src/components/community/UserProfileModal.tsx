...
import { X, UserPlus, UserCheck, Calendar, BookOpen, MessageSquare, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
      <div className="relative w-full max-w-2xl rounded-2xl bg-white border border-slate-200 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        {/* Header Cover Banner */}
        <div className="h-32 bg-linear-to-r from-indigo-600 via-sky-600 to-purple-600 relative shrink-0">
          <button
            onClick={onClose}
            className="absolute top-3 right-3 p-1.5 rounded-full bg-black/40 hover:bg-black/60 text-white transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 pb-4 border-b border-slate-100 relative shrink-0 bg-white">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-12 mb-3">
            ...
            {/* Avatar */}
            <div className="w-24 h-24 rounded-full bg-slate-900 border-4 border-white text-white flex items-center justify-center font-bold text-2xl shadow-md shrink-0">
              {user.avatar ? (
            ...
            <div>
              {isOwnProfile ? (
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-5 py-2 rounded-full text-xs font-bold shadow-xs active:scale-95 transition-all cursor-pointer ${
                    isFollowing
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              {user.role === 'admin' && (
                <span className="px-2 py-0.5 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 text-[10px] font-bold flex items-center gap-1">
